...
const getAllUsers = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, data: users, message: "Users found successfully" });
  } catch (err) {
    res.status(400).json({
      error: err.message,
...
const getUserById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, data: user, message: "User found successfully" });
  } catch (err) {
    res.status(400).json({
      error: err.message,
      message: "Your request cannot be processed. Please try again",
  ...
  try {
    ...
    if (!user.emailVerified) {
      return res.status(403).json({
        status: 403,
        message:
          "User account not verified. Please check your email to verify your account.",
      });
    }
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: "Invalid verification token or Email is already verified.",
        isUserVerified: false,
      });
    }
    ...
    if (tokenAge > twoDaysInMilliseconds) {
      return res.status(400).json({
        message:
          "Verification token is expired. Please request a new verification link.",
        isUserVerified: false,
      });
    }
