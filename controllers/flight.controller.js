...
const getFlightById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ status: 400, message: "Invalid flight id" });
    }
...
const createFlight = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ data: flight, message: "Flight created successfully" });
  } catch (err) {
    res.status(400).json({
      error: err.message,
      message: "Your request cannot be processed. Please try again",
...
const updateFlight = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(404)
        .json({ status: 404, message: "Invalid flight id" });
    }
    ...
    const updatedFlight = await Flight.findByIdAndUpdate(id, value, {
      new: true,
    });
...
const deleteFlight = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "Flight deleted successfully" });
  } catch (err) {
    res.status(404).json({
      error: err.message,
      message: "Your request cannot be processed. Please try again",
